...
import { createCategoryAPI, getAllCategoriesAPI, updateCategoryAPI, getCategoryPurchasersAPI, purchaseCategoryAPI, getPendingCategoryPurchasesAPI, approveCategoryPurchaseAPI, rejectCategoryPurchaseAPI } from "@/service/operations/category";
                      ...
                      <TableRow key={p._id}>
                        <TableCell>
                          <div className="flex flex-col">
                            <span className="font-medium">{p.vendor?.name || "Unknown"}</span>
                            <span className="text-xs text-gray-500">{p.vendor?.email || p.vendor?.phone || ""}</span>
                          </div>
                        </TableCell>
                        <TableCell>{p.category?.name || "-"}</TableCell>
                        <TableCell>₹{p.category?.price ?? "-"}</TableCell>
                        <TableCell className="capitalize">{p.paymentMode || "cash"}</TableCell>
                        <TableCell>{p.createdAt ? new Date(p.createdAt).toLocaleString() : (p.purchasedAt ? new Date(p.purchasedAt).toLocaleString() : "-")}</TableCell>
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            <Button size="sm" onClick={() => approve(p._id)} disabled={approvingId === p._id}> {approvingId === p._id ? "Approving..." : "Approve"} </Button>
                            <Button size="sm" variant="outline" onClick={() => reject(p._id)} disabled={rejectingId === p._id}> {rejectingId === p._id ? "Rejecting..." : "Reject"} </Button>
                          </div>
                        </TableCell>
                      </TableRow>
                    ))}
