...
import {
  verifyRedeemCode,
  applyRedeemCode,
  getVendorAppliedCodes,
  checkVendorRewardSettings,
...
const VendorApplyRedeemCode = () => {
  ...
  const fetchVendorSettings = async () => {
    try {
      const response = await checkVendorRewardSettings(token);
      if (response.success) {
  ...
  const fetchAppliedCodes = async () => {
    try {
      const response = await getVendorAppliedCodes(token, 1, 20);
      if (response.success) {
        setAppliedCodes(response.data);
      }
    } catch (error) {
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      const response = await verifyRedeemCode(token, code.toUpperCase());
      if (response.success) {
  ...
  const handleApplyCode = async (e?: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const response = await applyRedeemCode(token, code.toUpperCase());
      if (response.success) {
        setLastAppliedCode(response.data);
        setCode("");
        setCodeDetails(null);
        fetchAppliedCodes();
      }
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4">
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">Accept customer reward points and redeem codes</p>
        </div>
...
export default VendorApplyRedeemCode;
